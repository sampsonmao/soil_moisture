...
$$
\begin{aligned}
L(\beta, \psi, \sigma^2) &\propto \det(\sigma^2R(\psi))^{-1/2} \exp\left\{-\frac{1}{2}(X-D\beta)^T(\sigma^2R(\psi))^{-1}(X-D\beta)\right\} \\
&\propto \det(\sigma^2R(\psi))^{-1/2} \exp\left\{-\frac{1}{2}\left[(\beta-\hat{\beta})^TD^T(\sigma^2R(\psi))^{-1}D (\beta-\hat{\beta}) + S^2(\psi, \sigma^2)\right]\right\} \\
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-m/2} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta}) + S^2(\psi)\right]\right\} \\
\end{aligned}
$$
...
$$
\begin{aligned}
D^T(\sigma^2R(\psi))^{-1}D\hat{\beta} &= D^T(\sigma^2R(\psi))^{-1}X\\
S^2(\psi, \sigma^2) &= (X-D\hat{\beta})^T (\sigma^2R(\psi))^{-1}(X-D\hat{\beta})\\
S^2(\psi) &= (X-D\hat{\beta})^T R(\psi)^{-1}(X-D\hat{\beta})\\
\end{aligned}
$$

$$
\begin{aligned}
p(\beta,\psi,\sigma^2) &\propto p(\psi)IG(\sigma^2 | a,b) \\
p(\beta,\psi,\sigma^2 | \boldsymbol{x}) &\propto f(\boldsymbol{x} | \beta,\psi,\sigma^2) p(\beta,\psi,\sigma^2) \\
&\propto\det(R(\psi))^{-1/2}(\sigma^2)^{-m/2} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta}) + S^2(\psi)\right]\right\} (\sigma^2)^{-a-1} \exp \left\{-\frac{b}{\sigma^2}\right\} p(\psi)\\
&\propto\det(R(\psi))^{-1/2}(\sigma^2)^{-m/2-a-1} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta}) + S^2(\psi) + 2b\right]\right\} p(\psi) \\
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-m/2-a-1} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta})\right]\right\}  \exp\left\{-\frac{1}{2\sigma^2}\left[S^2(\psi) + 2b\right]\right\} p(\psi) \\
\end{aligned}
$$

$$
\begin{aligned}
p(\beta,\psi,\sigma^2 \mid \boldsymbol{x}) &= \pi(\beta \mid\psi,\sigma^2, \boldsymbol{x}) \, \pi(\sigma^2 \mid \psi, \boldsymbol{x}) \, \pi(\psi \mid \boldsymbol{x}) \\
p(\psi,\sigma^2 \mid \boldsymbol{x}) &\propto \int p(\beta,\psi,\sigma^2 \mid \boldsymbol{x}) \, d\beta \\ 
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-\frac{m}{2}-a-1} \det\left(\left(\frac{D^T R^{-1}(\psi)D}{\sigma^2}\right)^{-1}\right)^{1/2} \exp\left\{-\frac{1}{2\sigma^2}\left[S^2(\psi) + 2b\right]\right\} p(\psi) \\
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-\frac{m}{2}+\frac{k}{2}-a-1} \det(D^T R^{-1}(\psi)D)^{-1/2} \exp\left\{-\frac{1}{2\sigma^2}\left[S^2(\psi) + 2b\right]\right\} p(\psi) \\
\pi(\psi\mid\boldsymbol{x}) &= \frac{p(\psi,\sigma^2\mid\boldsymbol{x})}{\pi(\sigma^2 \mid \psi, \boldsymbol{x})} & \\
\pi(\beta \mid\psi,\sigma^2, \boldsymbol{x}) &\propto \exp\left\{-\frac{1}{2\sigma^2}(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta})\right\} \\
\pi(\sigma^2 \mid \psi, \boldsymbol{x}) &\propto(\sigma^2)^{-\frac{m}{2}+\frac{k}{2}-a-1} \exp\left\{-\frac{1}{2\sigma^2}\left[S^2(\psi) + 2b\right]\right\} \\
\pi(\psi \mid \boldsymbol{x}) &\propto \det(R(\psi))^{-1/2} \det(D^T R^{-1}(\psi)D)^{-1/2} \left(S^2(\psi)+2b\right)^{-\frac{m-k}{2}-a}  p(\psi)
\end{aligned}
$$

To use the conditional probability formula in line 5, we need the exact form (not proportionality) of $\pi(\sigma^2\mid\psi,x)$ since we need all factors of $\psi$

$$
\begin{aligned}
p(\beta,\psi,\sigma^2) &\propto 1/\sigma^2 \pi^R(\psi) \\
\pi^R(\psi) &\propto \left\{tr[H_{\psi}^2] - \frac{1}{n-p}(tr[H_\psi])^2 \right\}\\
H_\psi &= \left(\frac{\partial}{\partial \psi} R_\psi\right) R_{\psi}^{-1}P_\psi^R\\ 
P_\psi^R &= I-D(D^TR_\psi^{-1}D)^{-1}D^TR_\psi^{-1}\\
p(\beta,\psi,\sigma^2 | \boldsymbol{x}) &\propto f(\boldsymbol{x} | \beta,\psi,\sigma^2) p(\beta,\psi,\sigma^2) \\
&\propto\det(R(\psi))^{-1/2}(\sigma^2)^{-m/2} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta}) + S^2(\psi)\right]\right\} (\sigma^2)^{-1}  \pi^R(\psi)\\
&\propto\det(R(\psi))^{-1/2}(\sigma^2)^{-m/2-1} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta}) + S^2(\psi)\right]\right\}  \pi^R(\psi) \\
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-m/2-1} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta})\right]\right\}  \exp\left\{-\frac{1}{2\sigma^2}\left[S^2(\psi)\right]\right\}  \pi^R(\psi) \\
\end{aligned}
$$
...
p(\beta,\psi,\sigma^2 \mid \boldsymbol{x}) &= \pi(\beta \mid\psi,\sigma^2, \boldsymbol{x}) \, \pi(\sigma^2 \mid \psi, \boldsymbol{x}) \, \pi(\psi \mid \boldsymbol{x}) \\
p(\psi,\sigma^2 \mid \boldsymbol{x}) &\propto \int p(\beta,\psi,\sigma^2 \mid \boldsymbol{x}) \, d\beta \\ 
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-\frac{m}{2}-1} \det\left(\left(\frac{D^T R^{-1}(\psi)D}{\sigma^2}\right)^{-1}\right)^{1/2} \exp\left\{-\frac{1}{2\sigma^2}S^2(\psi)\right\}  \pi^R(\psi) \\
&\propto \det(R(\psi))^{-1/2}(\sigma^2)^{-\frac{m}{2}+\frac{k}{2}-1} \det(D^T R^{-1}(\psi)D)^{-1/2} \exp\left\{-\frac{1}{2\sigma^2}S^2(\psi)\right\}  \pi^R(\psi) \\
\pi(\psi\mid\boldsymbol{x}) &= \frac{p(\psi,\sigma^2\mid\boldsymbol{x})}{\pi(\sigma^2 \mid \psi, \boldsymbol{x})}\\
\pi(\beta \mid\psi,\sigma^2, \boldsymbol{x}) &\propto \exp\left\{-\frac{1}{2\sigma^2}(\beta-\hat{\beta})^TD^T R^{-1}(\psi)D (\beta-\hat{\beta})\right\} \\
\pi(\sigma^2 \mid \psi, \boldsymbol{x}) &\propto(\sigma^2)^{-\frac{m}{2}+\frac{k}{2}-1} \exp\left\{-\frac{1}{2\sigma^2}S^2(\psi) \right\} \\
\pi(\psi \mid \boldsymbol{x}) &\propto \det(R(\psi))^{-1/2} \det(D^T R^{-1}(\psi)D)^{-1/2} \left(S^2(\psi)+2b\right)^{-\frac{m-k}{2}-a}  \pi^R(\psi)
\end{aligned}
$$
$$
\begin{aligned}
L(\beta, \sigma^2, \tau^2, \phi, \nu) 
&\propto \det(\sigma^2 R(\phi, \nu) + \tau^2I)^{-1/2} \exp\left\{-\frac{1}{2}(X-D\beta)^T(\sigma^2 R(\phi, \nu) + \tau^2I)^{-1}(X-D\beta)\right\} \\
L(\beta, \sigma^2, \gamma^2, \phi, \nu) 
&\propto \det(R(\phi, \nu) + \gamma^2I)^{-1/2} (\sigma^2)^{-m/2} \exp\left\{-\frac{1}{2\sigma^2}(X-D\beta)^T(R(\phi, \nu) + \gamma^2I)^{-1}(X-D\beta)\right\} \\
&\propto \det(V(\phi, \nu, \gamma^2))^{-1/2} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T V(\phi, \nu, \gamma^2)^{-1} D (\beta-\hat{\beta}) + S^2(\phi, \nu)\right]\right\} \\
\end{aligned}
$$
...
\begin{aligned}
V(\phi, \nu, \gamma^2) &= R(\phi, \nu) + \gamma^2I\\
D^TV(\phi, \nu, \gamma^2)^{-1}D\hat{\beta} &= D^TV(\phi, \nu, \gamma^2)^{-1}X\\
S^2(\gamma^2, \phi, \nu) &= (X-D\hat{\beta})^T V(\phi, \nu, \gamma^2)^{-1}(X-D\hat{\beta})\\
\end{aligned}
$$

$$
\begin{aligned}
p(\beta, \sigma^2, \gamma^2, \phi, \nu) &= p(\beta, \sigma^2|  \gamma^2, \phi, \nu) p(\gamma^2, \phi, \nu) \\
p(\beta, \sigma^2, \gamma^2, \phi, \nu) &= IG(\sigma^2 \mid a,b) p(\gamma^2, \phi, \nu) \\
p(\beta, \sigma^2, \gamma^2, \phi, \nu| \boldsymbol{x}) &\propto f(\boldsymbol{x} | \beta, \sigma^2, \gamma^2, \phi, \nu) p(\beta, \sigma^2, \gamma^2, \phi, \nu) \\
&\propto \det(V(\phi, \nu, \gamma^2))^{-1/2} \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T V(\phi, \nu, \gamma^2)^{-1} D (\beta-\hat{\beta}) + S^2(\gamma^2, \phi, \nu)\right]\right\} \\
&\phantom{\propto} (\sigma^2)^{-\frac{m-k}{2}-a-1} \exp\left\{-\frac{b}{\sigma^2}\right\}
 p(\gamma^2)p(\phi)p(\nu) \\
p(\sigma^2, \gamma^2, \phi, \nu| \boldsymbol{x}) 
&\propto \det(V(\phi, \nu, \gamma^2))^{-1/2} \det(D^TV(\phi, \nu, \gamma^2)^{-1}D)^{-1/2} \exp\left\{- \frac{1}{2\sigma^2} S^2(\gamma^2, \phi,\nu)\right\}\\
&\phantom{\propto} (\sigma^2)^{-\frac{m-k}{2}-a-1} \exp\left\{-\frac{b}{\sigma^2}\right\}  p(\gamma^2)p(\phi)p(\nu) \\
p(\gamma^2, \phi, \nu| \boldsymbol{x}) 
&\propto \det(V(\phi, \nu, \gamma^2))^{-1/2} \det(D^TV(\phi, \nu, \gamma^2)^{-1}D)^{-1/2} (\sigma^2)^{-\frac{m-k}{2}-a-1} (S(\gamma^2, \phi, \nu)+2b)^{-\frac{m-k}{2}-a} p(\gamma^2, \phi, \nu)
\end{aligned}
$$

$$
\begin{aligned}
p(\beta | \sigma^2, \tau^2, \phi, \nu, \boldsymbol{x}) 
&\propto \exp\left\{-\frac{1}{2\sigma^2}\left[(\beta-\hat{\beta})^TD^T V(\phi, \nu, \gamma^2)^{-1} D (\beta-\hat{\beta}) + S^2(\gamma^2, \phi, \nu)\right]\right\} \\
p(\sigma^2 | \gamma^2, \phi, \nu, \boldsymbol{x}) 
&= (\sigma^2)^{-\frac{m-k}{2}-a-1} \exp\left\{-\frac{S^2(\gamma^2, \phi, \nu) +2b}{2\sigma^2}\right\} \\
p(\gamma^2, \phi, \nu| \boldsymbol{x}) 
&\propto |V(\phi, \nu, \gamma^2)|^{-1/2} |D^TV(\phi, \nu, \gamma^2)^{-1}D|^{-1/2} (\sigma^2)^{-\frac{m-k}{2}-a-1} (S(\gamma^2, \phi, \nu)+2b)^{-\frac{m-k}{2}-a} p(\gamma^2, \phi, \nu)
\end{aligned}
$$

```{r}
logL = function(sigma2, phi, nu=0.5, tau2) {
  V.mat = varcov.spatial(in_geor_jitter$coords, cov.pars=c(sigma2, phi), nugget=tau2, kappa=nu)
  V = V.mat$varcov
